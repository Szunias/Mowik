...
> [!WARNING]
> The official Mówik installer is **not digitally signed**. Windows may therefore show **Unknown publisher** or a Microsoft Defender SmartScreen warning. Download the installer only from the [official Mówik GitHub release](https://github.com/Szunias/Mowik/releases/latest) and verify its SHA-256 hash against `SHA256SUMS.txt` from the same release before running it. In PowerShell, use `Get-FileHash .\Mowik-2.9.0-Setup-UNSIGNED.exe -Algorithm SHA256` and compare the complete value. Do not disable Windows security protections to install Mówik.

Official release builds also receive a signed GitHub/Sigstore provenance attestation. With GitHub CLI installed, verify the downloaded installer using `gh attestation verify .\Mowik-2.9.0-Setup-UNSIGNED.exe --repo Szunias/Mowik`. This authenticates the GitHub Actions build identity even though the Windows executable itself remains unsigned.
...
Mówik uses its own pinned CUDA 12.9 and cuBLAS runtime, so it does not depend on CUDA libraries installed by other applications. Those libraries are not part of the installer: they weigh close to a gigabyte and only NVIDIA owners need them, so Mówik downloads them from the Python Package Index the first time a model starts on the GPU, verifies them against the checksums pinned in the release, and stores them in `%LOCALAPPDATA%\Mowik\cuda-12.9`. Without a network connection at that moment, dictation starts on the processor and says so. The bundled CTranslate2 handles Whisper convolution without cuDNN, so that unused package is not included. A compatible NVIDIA GPU is selected automatically; CUDA processing uses `float16`, while the automatic CPU fallback uses `int8`. The bundled CUDA runtime supports RTX 50-series GPUs. If the GPU encoder test fails, Mówik records the technical details in the log and continues on the CPU.
